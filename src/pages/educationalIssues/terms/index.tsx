...
import TermLoadingCard from "../../../components/pages/educationalIssues/educationalTerms/termLoadingCard";
import TermCard from "../../../components/pages/educationalIssues/educationalTerms/termCard";
import AddAndEditTermModal from "../../../components/pages/educationalIssues/educationalTerms/addAndEditTermModal";
...
const Terms = () => {
  ...
  const [isShowAddTerm, setIsShowAddTerm] = useState(false);
  ...
  const generateList = () => {
    ...
    return terms.data.map((singleTerm, index) => (
      <TermCard key={index} index={index} singleTerm={singleTerm} />
    ));
  };

  const loadingCard = () => {
    return <TermLoadingCard />;
  };
  ...
  return (
    <div className="my-10">
      ...
      {isShowAddTerm && (
        <AddAndEditTermModal
          isShow={isShowAddTerm}
          modalType="add"
          onCloseModal={() => setIsShowAddTerm(false)}
        />
      )}
    </div>
  );
